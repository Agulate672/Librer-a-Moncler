"""
URL configuration for src project.

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/5.1/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path
from . import views
from biblioteca import views as biblioteca
from contactenos import views as contactenos

urlpatterns = [
    path('admin/', admin.site.urls),
    path('', views.index, name='index'),
    path('autores/', biblioteca.autores, name='autores' ), 
    path('libros/', biblioteca.libros, name='libros'),
    path('contactenos/', contactenos.contactenos),
    path('crear_autor/', biblioteca.crear_autor),
    path('editar_autor/<int:id>', biblioteca.editar_autor, name ='editar_autor' ),
    path('accion_autor/<int:id>', biblioteca.accion_autor, name ='accion_autor'),
    path('eliminar_autor/<int:id>/', biblioteca.eliminar_autor, name='eliminar_autor'),
    path('eliminar_libro/<int:id>/', biblioteca.eliminar_libro, name='eliminar_libro'),
    path('crear_libro/', biblioteca.crear_libro, name='crear_libro'),
    path('editar_libro/<int:id>/', biblioteca.editar_libro, name ='editar_libro' ),

]
